...
class NeighborFinder:
    def __init__(self, adj_list, walk_type, bias=0, ts_precision=PRECISION, sample_method='multinomial'):
        ...
        self.bias = bias 
        node_idx_l, node_ts_l, edge_idx_l, sign_idx_l, weight_idx_l, direction_idx_l, binary_prob_l, off_set_l, self.nodeedge2idx = self.init_off_set(adj_list)
        self.node_idx_l = node_idx_l
        self.node_ts_l = node_ts_l
        self.edge_idx_l = edge_idx_l
        self.sign_idx_l = sign_idx_l 
        self.weight_idx_l = weight_idx_l
        self.direction_idx_l = direction_idx_l 
    ...
    def init_off_set(self, adj_list):
        ...
        for i in range(len(adj_list)):
            ...
            sign_l.extend([x[3] for x in curr])
            weight_l.extend([x[4] for x in curr])
            direction_l.extend([x[5] for x in curr])  
        ...
        sign_l = np.array(sign_l)
        weight_l = np.array(weight_l)
        direction_l = np.array(direction_l)  
        binary_prob_l = np.concatenate(binary_prob_l)
        off_set_l = np.array(off_set_l)

        assert(len(n_idx_l) == len(n_ts_l))
        assert(off_set_l[-1] == len(n_ts_l))
        return n_idx_l, n_ts_l, e_idx_l, sign_l, weight_l, direction_l, binary_prob_l, off_set_l, nodeedge2idx 
    ...
    def get_ts2idx(self, sorted_triples):
        ...
        last_ts = -1
        last_e_idx = -1
        for i, (n_idx, e_idx, ts_idx, sign_idx, weight, direction) in enumerate(sorted_triples):
            ts2idx[e_idx] = i
    ...
    def find_before(self, src_idx, cut_time, e_idx=None, return_binary_prob=False):
        ...
        sign_idx_l = self.sign_idx_l
        weight_idx_l = self.weight_idx_l
        direction_idx_l = self.direction_idx_l 
        ...
        neighbors_s_idx = sign_idx_l[start: end]
        neighbors_w_idx = weight_idx_l[start: end]
        neightbors_d_idx = direction_idx_l[start: end] 

        assert (len(neighbors_idx) == len(neighbors_ts) and len(neighbors_idx) == len(neighbors_e_idx) and len(neighbors_idx) == len(neighbors_s_idx) and len(neighbors_idx) == len(neighbors_w_idx))
        if e_idx is None:
        ...
        elif self.walk_type  == 'point':
            cut_idx = get_cut_idx_point(neighbors_ts, cut_time) if src_idx > 0 else 0
            if cut_idx is None:
                raise IndexError('e_idx {} not found in edge list of {}'.format(e_idx, src_idx))

        if not return_binary_prob:
            result = (neighbors_idx[:cut_idx], neighbors_e_idx[:cut_idx], neighbors_ts[:cut_idx], neighbors_s_idx[:cut_idx], neighbors_w_idx[:cut_idx], neightbors_d_idx[:cut_idx], None)
        else:
            neighbors_binary_prob = binary_prob_l[start: end]
            result = (neighbors_idx[:cut_idx], neighbors_e_idx[:cut_idx], neighbors_ts[:cut_idx], neighbors_s_idx[:cut_idx], neighbors_w_idx[:cut_idx], neightbors_d_idx[:cut_idx], neighbors_binary_prob[:cut_idx])
    ...
    def get_temporal_neighbor(self, src_idx_l, cut_time_l, num_neighbor=20, e_idx_l=None):
        ...
        out_ngh_sign_batch = np.zeros((len(src_idx_l), num_neighbor)).astype(np.int32)  
        out_ngh_weight_batch = np.zeros((len(src_idx_l), num_neighbor)).astype(np.int32)    
        out_ngh_direction_batch = np.zeros((len(src_idx_l), num_neighbor)).astype(np.int32) 

        for i, (src_idx, cut_time) in enumerate(zip(src_idx_l, cut_time_l)):
            
            ngh_idx, ngh_eidx, ngh_ts, ngh_s, ngh_w, ngh_d, ngh_binomial_prob = self.find_before(src_idx, cut_time, e_idx=e_idx_l[i] if e_idx_l is not None else None,
            ...
            out_ngh_sign_batch[i, :] = ngh_s[sampled_idx]
            out_ngh_weight_batch[i, :] = ngh_w[sampled_idx]
            out_ngh_direction_batch[i, :] = ngh_d[sampled_idx]
        return out_ngh_node_batch, out_ngh_eidx_batch, out_ngh_t_batch, out_ngh_sign_batch, out_ngh_weight_batch, out_ngh_direction_batch
      
    

    def find_k_hop(self, k, src_idx_l, cut_time_l, label_l_cut, num_neighbors, e_idx_l=None):
        if k == 0:
            return ([], [], [])
        batch = len(src_idx_l)
        layer_i = 0
        x, y, z, s, w, d = self.get_temporal_neighbor(src_idx_l, cut_time_l, num_neighbors, e_idx_l=e_idx_l) 
        node_records = [x]
        eidx_records = [y]
        t_records = [z]
        sign_records = [s]
        ...
        for layer_i in range(1, k):
            ngh_node_est, ngh_e_est, ngh_t_est, ngh_sign_est, ngh_weight_est = node_records[-1], eidx_records[-1], t_records[-1], sign_records[-1], weight_records[-1]
            ngh_node_est = ngh_node_est.flatten()
            ngh_e_est = ngh_e_est.flatten()
            ngh_t_est = ngh_t_est.flatten()
            ngh_sign_est = ngh_sign_est.flatten()
            ngh_weight_est = ngh_weight_est.flatten()
            if self.walk_type == 'point':
                out_ngh_node_batch, out_ngh_eidx_batch, out_ngh_t_batch, out_ngh_sign_batch, out_ngh_weight_batch, out_ngh_direction_batch = self.get_temporal_neighbor(ngh_node_est, cut_time_l, 1, e_idx_l=ngh_e_est)
            elif self.walk_type == 'before':
                out_ngh_node_batch, out_ngh_eidx_batch, out_ngh_t_batch, out_ngh_sign_batch, out_ngh_weight_batch, out_ngh_direction_batch = self.get_temporal_neighbor(ngh_node_est, ngh_t_est, 1, e_idx_l=ngh_e_est)
            ...
            out_ngh_sign_batch = out_ngh_sign_batch.reshape(batch, -1)
            out_ngh_weight_batch = out_ngh_weight_batch.reshape(batch, -1)
            out_ngh_direction_batch = out_ngh_direction_batch.reshape(batch, -1)

            node_records.append(out_ngh_node_batch)
            eidx_records.append(out_ngh_eidx_batch)
            t_records.append(out_ngh_t_batch)
            sign_records.append(out_ngh_sign_batch)
            weight_records.append(out_ngh_weight_batch)
            direction_records.append(out_ngh_direction_batch) 
        return (node_records, eidx_records, t_records, sign_records, weight_records, direction_records) 
